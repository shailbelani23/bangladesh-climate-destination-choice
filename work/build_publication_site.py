...
<section class="abstract" aria-labelledby="abstract-heading"><h2 id="abstract-heading">Abstract</h2><p>Most climate-migration research asks whether households leave. This study asks what happens after they move: which district becomes the destination? I compare recorded moves in the Bangladesh Environmental Mobility Panel and the Bangladesh Integrated Household Survey with the same 64 candidate districts. The baseline model uses distance and population. A second model adds flood history, built surface, travel time to a city, and cropland. On held-out observations, those four measures improve destination prediction by 0.108 log-loss units for 184 shock-linked BEMP relocations and 0.098 for 123 BIHS moves attributed to river-erosion land loss. The estimates are predictive, and the public locations stop at the district level.</p></section>
...
<h2>The gain appears in both household surveys</h2><p>For 184 BEMP relocations that follow a recorded flood or erosion shock, the GIS model improves held-out log loss by 0.108. For 123 BIHS moves attributed to river-erosion land loss, the gain is 0.098.</p><div class="figure"><img src="assets/forest.png" alt="Eight held-out comparisons between a distance-and-population model and a model that adds district GIS measures. All eight point estimates are positive. The BEMP interdistrict interval crosses zero; the other seven displayed intervals are above zero."><p class="caption">Each dot is the average held-out gain over the distance-and-population model. Lines show paired 95 percent household-cluster bootstrap intervals.</p></div>
<h2>The broad BIHS sample covers every origin district</h2><p>The wider BIHS sample contains 1,857 migrants from all 64 origin districts. The gain remains about 0.10 when the model leaves each origin out of training. The climate-specific samples are much smaller under that test, so their origin-held-out estimates are less precise.</p><div class="figure"><img src="assets/transport.png" alt="Comparison of household-grouped, origin-held-out, and later-wave tests. The broad BIHS migrant estimates stay near a 0.10 gain. Climate-specific full-choice estimates are weaker when an entire origin is excluded."><p class="caption">Destination rankings transfer across origins more reliably than boundary-crossing predictions. Whether a household crosses a district boundary still depends on local origin conditions.</p></div>
...
<footer>Shail Belani, Northwestern University and the Global Poverty Research Lab. All results use public district-level geography and are conditional on a recorded move.</footer>
"""
